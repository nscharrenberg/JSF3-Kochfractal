 ...
 */
public class TaskBottom extends Task<List<Edge>> implements Observer {
    ...
    public TaskBottom(KochFractal k, KochManager km) {
        this.k = k;
        this.km = km;
        k.addObserver(this);
    ...
    private void changeProgressBarFromTask() {
        final ProgressBar pb = km.getApplication().getProgressBarBottom();
        final Label lb = km.getApplication().getLabelBottom();
        ...
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                pb.progressProperty().bind(progProp);
                lb.textProperty().bind(mesProp);
            }
